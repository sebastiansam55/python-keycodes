...
@pytest.mark.parametrize("hotkey, result",
    [
    ([["LeftControl", "LeftShift"],"a"], [[29, 42], 30])
])
def test_hotkey_to_evdev(hotkey, result):
    assert result == keycodes.hotkey_to_evdev(hotkey)
